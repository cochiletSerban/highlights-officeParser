...
const ERRORMSG = {
  extensionUnsupported: (ext) =>
    `Sorry, OfficeParser currently support docx, pptx, xlsx, odt, odp, ods, pdf files only. Create a ticket in Issues on github to add support for ${ext} files. Stay tuned for further updates.`,
  fileCorrupted: (filepath) =>
...
function consoleError(errorMessage, outputErrorToConsole) {
  if (!errorMessage || !outputErrorToConsole) return;
...
function parseWord(filepath, callback, config) {
  ...
  const decompressLocation = `${config.tempFilesLocation}/${filepath
    .split("/")
    .pop()}`;

  decompress(filepath, decompressLocation, {
    filter: (x) =>
      [mainContentFileRegex, footnotesFileRegex, endnotesFileRegex].some(
        (fileRegex) => x.path.match(fileRegex)
      ),
  })
    .then((files) => {
      ...
      return files
        .filter(
          (file) =>
            file.path.match(mainContentFileRegex) ||
            file.path.match(footnotesFileRegex) ||
            file.path.match(endnotesFileRegex)
        )
        .map((file) =>
          fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
        );
    })
    ...
    .then((xmlContentArray) => {
      ...
      xmlContentArray.forEach((xmlContent) => {
        ...
        const xmlParagraphNodesList =
          parseString(xmlContent).getElementsByTagName("w:p");
        /** Store all the text content to respond */
        responseText.push(
      ...
      const data = {
        responseText,
        filePathToDelete: decompressLocation,
      };
...
function parsePowerPoint(filepath, callback, config) {
  ...
  decompress(filepath, decompressLocation, {
    filter: (x) =>
      x.path.match(config.ignoreNotes ? slidesRegex : allFilesRegex),
  })
    .then((files) => {
    ...
    .then((xmlContentArray) => {
      ...
      xmlContentArray.forEach((xmlContent) => {
        ...
        const xmlParagraphNodesList =
          parseString(xmlContent).getElementsByTagName("a:p");
        /** Store all the text content to respond */
        responseText.push(
          Array.from(xmlParagraphNodesList)
  ...
  decompress(filepath, decompressLocation, {
    filter: (x) =>
      [sheetsRegex, drawingsRegex, chartsRegex].some((fileRegex) =>
        x.path.match(fileRegex)
      ) || x.path == stringsFilePath,
  })
    .then((files) => {
      // Verify if atleast the slides xml files exist in the extracted files list.
      if (
      ...
      return {
        sheetFiles: files
          .filter((file) => file.path.match(sheetsRegex))
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          ),
        drawingFiles: files
          .filter((file) => file.path.match(drawingsRegex))
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          ),
        chartFiles: files
          .filter((file) => file.path.match(chartsRegex))
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          ),
        sharedStringsFile: files
          .filter((file) => file.path == stringsFilePath)
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          )[0],
      };
    ...
    .then((xmlContentFilesObject) => {
      ...
      const sharedStringsXmlTNodesList = parseString(
        xmlContentFilesObject.sharedStringsFile
      ...
      xmlContentFilesObject.sheetFiles.forEach((sheetXmlContent) => {
        ...
        const sheetsXmlCNodesList =
          parseString(sheetXmlContent).getElementsByTagName("c");
        ...
        responseText.push(
          Array.from(sheetsXmlCNodesList)
            ...
            .map((cNode) => {
              ...
              return isString ? sharedStrings[value] : value;
            })
      ...
      xmlContentFilesObject.drawingFiles.forEach((drawingXmlContent) => {
        ...
        const drawingsXmlParagraphNodesList =
          parseString(drawingXmlContent).getElementsByTagName("a:p");
        /** Store all the text content to respond */
        responseText.push(
      ...
      xmlContentFilesObject.chartFiles.forEach((chartXmlContent) => {
        ...
        const chartsXmlCVNodesList =
          parseString(chartXmlContent).getElementsByTagName("c:v");
        /** Store all the text content to respond */
        responseText.push(
          Array.from(chartsXmlCVNodesList)
            .filter(
              (cVNode) => cVNode.childNodes[0] && cVNode.childNodes[0].nodeValue
            )
...
function parseOpenOffice(filepath, callback, config) {
  ...
  decompress(filepath, decompressLocation, {
    filter: (x) =>
      x.path == mainContentFilePath || x.path.match(objectContentFilesRegex),
  })
    .then((files) => {
      ...
      return {
        mainContentFile: files
          .filter((file) => file.path == mainContentFilePath)
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          )[0],
        objectContentFiles: files
          .filter((file) => file.path.match(objectContentFilesRegex))
          .map((file) =>
            fs.readFileSync(`${decompressLocation}/${file.path}`, "utf8")
          ),
      };
    ...
    .then((xmlContentFilesObject) => {
      ...
      function isNotesNode(node) {
        if (node.tagName == notesTag) return true;
      ...
      function isInvalidTextNode(node) {
        if (allowedTextTags.includes(node.tagName)) return true;
      ...
      const xmlContentArray = [
        xmlContentFilesObject.mainContentFile,
        ...xmlContentFilesObject.objectContentFiles,
      ].map((xmlContent) => parseString(xmlContent));
...
function parseOffice(file, callback, config = {}) {
  ...
  const filePreparedPromise = new Promise((res, rej) => {
    ...
    if (!internalConfig.tempFilesLocation)
      ...
      internalConfig.tempFilesLocation = `${internalConfig.tempFilesLocation}${
        internalConfig.tempFilesLocation.endsWith("/") ? "" : "/"
      }${DEFAULTDECOMPRESSSUBLOCATION}`;
    }

    // create temp file subdirectory if it does not exist

    fs.mkdirSync(`${internalConfig.tempFilesLocation}/tempfiles`, {
      recursive: true,
    });
    ...
    if (Buffer.isBuffer(file)) {
      ...
      fileType
        ...
        .then((data) => {
          // temp file name
          const newfilepath = getNewFileName(
            internalConfig.tempFilesLocation,
    ...
    .then((filepath) => {
      ...
      function internalCallback(data, err) {
        ...
        if (!internalConfig.preserveTempFiles) {
          ...
          rimraf(data.filePathToDelete, (rimrafErr) =>
            consoleError(rimrafErr, internalConfig.outputErrorToConsole)
          );
          rimraf(
            "officeParserTemp/tempfiles/" + data.filePathToDelete.split("/")[1],
            (rimrafErr) =>
              consoleError(rimrafErr, internalConfig.outputErrorToConsole)
          );
        }
        ...
        callback(data.responseText, undefined);
      }
    })
    .catch((error) =>
      handleError(error, callback, internalConfig.outputErrorToConsole)
    );
}
...
function getNewFileName(tempFilesLocation, ext) {
  ...
  if (globalFileNameIterator > 99999) globalFileNameIterator = 0;

  // Return the file name
  return `${tempFilesLocation}/tempfiles/${
    new Date().getTime().toString() + iteratorPart
  }.${ext}`;
}
...
if (
  (process.argv[0].split("/").pop() == "node" ||
    process.argv[0].split("/").pop() == "npx") &&
  (process.argv[1].split("/").pop() == "officeParser.js" ||
    process.argv[1].split("/").pop() == "officeparser")
) {
  ...
  else console.error(ERRORMSG.improperArguments);
}
